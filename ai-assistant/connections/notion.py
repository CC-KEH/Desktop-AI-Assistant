...
class Notion:
    def __init__(self):
        ...
        self.todo_payload = {
            "properties": {
                ...
                "Priority": {"multi_select": [{"text": {"content": "priority"}}]},
                "Completed": {"checkbox": "completed"},
            }
        }
    ...
    def create_item(self, name, multivalue, asked_for, completed=False):
        if asked_for == "todos":
            ...
            payload["properties"]["Priority"]["multi_select"][0]["text"][
                "content"
            ] = multivalue
    ...
    def update_item(self, item, asked_for):
        ...
        if asked_for == "todos":
            ...
            self.todo_payload["properties"]["Task"]["title"][0]["text"]["content"] = (
                item["task"]
            )
            self.todo_payload["properties"]["Priority"]["multi_select"][0]["text"][
                "content"
            ] = item["priority"]
            ...
            self.watch_list_payload["properties"]["Name"]["title"][0]["text"][
                "content"
            ] = item["name"]
            self.watch_list_payload["properties"]["Genre"]["multi_select"][0][
                "name"
            ] = item["genre"]
            self.watch_list_payload["properties"]["Completed"]["checkbox"] = item[
                "completed"
            ]
...
if __name__ == "__main__":
    ...
    task = {"task": "task2", "priority": "medium", "completed": True}
